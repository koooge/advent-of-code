...
fn map_steps_rev(
    ...
    if y > 0
        && steps[y - 1][x] > steps[y][x] + 1
        && ((heights[y][x] >= 'a' && heights[y - 1][x] as u8 >= yxu8 - 1)
            || heights[y][x] == 'E' && heights[y - 1][x] == 'z'
            || heights[y][x] == 'a' && heights[y - 1][x] == 'S')
    {
        steps[y - 1][x] = steps[y][x] + 1;
        map_steps_rev(heights, steps, y_len, x_len, y - 1, x);
    }

    if x + 1 < x_len
        && steps[y][x + 1] > steps[y][x] + 1
        && ((heights[y][x] >= 'a' && heights[y][x + 1] as u8 >= yxu8 - 1)
            || heights[y][x] == 'E' && heights[y][x + 1] == 'z'
            || heights[y][x] == 'a' && heights[y][x + 1] == 'S')
    {
        steps[y][x + 1] = steps[y][x] + 1;
        map_steps_rev(heights, steps, y_len, x_len, y, x + 1);
    }
...
pub fn solve_part2(inputs: Vec<String>) -> u32 {
    ...
    let map_size: (usize, usize) = (heights.len(), heights[0].len());
    let mut steps: Vec<Vec<u32>> = vec![vec![u32::MAX; map_size.1]; map_size.0];
    steps[pos_e.0][pos_e.1] = 0;

    map_steps_rev(
        &heights, &mut steps, map_size.0, map_size.1, pos_e.0, pos_e.1,
    );
    ...
    for i in 0..heights.len() {
        for j in 0..heights[i].len() {
            if heights[i][j] == 'a' {
                min = if steps[i][j] < min { steps[i][j] } else { min };
            }
        }
    }
    min
